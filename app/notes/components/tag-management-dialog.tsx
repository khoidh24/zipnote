...
import { addTag, deleteTag, getAllTags, updateTag } from "@/lib/tags-db";
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-125">
          <DialogHeader>
            ...
            <DialogDescription>
              Create and manage your tags for better organization.
            </DialogDescription>
          </DialogHeader>
                          ...
                          {TAG_COLORS.map((color) => (
                            <button
                              ...
                              className={`h-7 w-full rounded ${
                                color.value
                              } ring-1 ring-offset-1 transition-all ${
                  ...
                  <div className="flex flex-wrap gap-2 py-2">
                    {tags.map((tag) => (
                      <Badge
                        key={tag.id}
                        variant={
                          editingTag?.id === tag.id ? "default" : "secondary"
                        }
                        className="group relative pr-8 cursor-pointer hover:opacity-80 transition-all"
                        onClick={() => handleEditClick(tag)}
                      >
                        <span
                          className={`inline-block w-2 h-2 rounded-full mr-1.5 ${tag.color}`}
                        />
                        {tag.name}
