...
import { deleteStatus, deleteTask, getAllStatuses, getAllTasks, updateStatus, updateTask } from '@/lib/db'
...
export default function CheckListPage() {
  ...
  const [deleteStatusConfirm, setDeleteStatusConfirm] = useState<{ open: boolean; statusId?: string }>({
    open: false
  })
  ...
  const loadData = async () => {
    try {
      setIsLoading(true)
      const [loadedStatuses, loadedTasks] = await Promise.all([getAllStatuses(), getAllTasks()])
  ...
  const handleDragOver = (event: DragOverEvent) => {
    ...
    if (isActiveATask && isOverAStatus) {
      setTasks((tasks) => {
        const activeIndex = tasks.findIndex((t) => t.id === activeId)
        tasks[activeIndex].statusId = overId as string
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (activeType === 'Status') {
      ...
      if (activeIndex !== overIndex) {
        ...
        try {
          await Promise.all(newStatuses.map((status, index) => updateStatus(status.id, { order: index })))
        } catch (error) {
    ...
    if (activeType === 'Task') {
      ...
      if (task) {
        try {
          await updateTask(task.id, { statusId: task.statusId, order: Date.now() })
        } catch (error) {
    ...
    <div className='flex flex-col h-full p-6 gap-4'>
      ...
      <div className='flex items-center justify-between'>
        <div>
          ...
          <p className='text-muted-foreground text-sm mt-1'>Organize your tasks with drag and drop</p>
        </div>
              ...
              {statuses.length === 0 && (
                <div className='flex items-center justify-center h-full w-full'>
                  <div className='text-center'>
                    <p className='text-muted-foreground mb-4'>No status columns yet</p>
                    <Button onClick={() => setCreateStatusDialogOpen(true)}>
                      <Plus className='h-4 w-4 mr-2' />
              ...
              {activeTask ? (
                ...
                <div className='bg-muted/80 backdrop-blur-sm px-4 py-2 rounded-lg shadow-lg border-2 border-primary'>
                  <div className='flex items-center gap-2'>
                    <div className={cn('w-3 h-3 rounded-full', activeStatus.color)} />
                    <span className='font-semibold text-sm'>{activeStatus.title}</span>
                  </div>
                </div>
              ) : null}
            </DragOverlay>
